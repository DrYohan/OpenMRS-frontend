...
const Vehicle = () => {
  ...
  useEffect(() => {
    const fetchAllSuppliers = async () => {
      try {
        const response = await axios.get("http://localhost:3000/api/vehicle/names")
...
/*   useEffect(() => {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const [middleResponse, subResponse] = await Promise.all([
        axios.get("http://localhost:3000/api/asset-categories/middle-categories"),
  ...
  const fetchRegistrationId = async () => {
    try {
      const response = await axios.get("http://localhost:3000/api/vehicle/registrationId");
      console.log("response regiatration ids ", response.data.data);
      setRegistrationIds(        response.data.data.map(d => ({
  ...
  const fetchVehicleByRegistrationId = async (registrationId) => {
    try {
      ...
      setFormData(prev => ({
        ...
        leasePeriodStartDate: vehicle.lease_period_start_date ? vehicle.lease_period_start_date.split("T")[0]: "",
        leasePeriodEndDate: vehicle.lease_period_end_date ? vehicle.lease_period_end_date.split("T")[0]: "",
        ...
        purchasedDate: vehicle.purchased_date ? vehicle.purchased_date.split("T")[0]: "",
      ...
      const vehicleFiles = vehicle.file
        ...
        const previews = vehicleFiles.map(f => f.serverPath ? `http://localhost:3000/${f.serverPath}` : null);
  ...
  const fetchMiddleCategoriesByStation = async (stationId) => {
    try {
      ...
      console.log("fetchMiddleCategoriesByStation response.data.data", response.data.data)
      setMiddleCategoryOptions(
  ...
  // fetch R_ID by 
  useEffect(()=>{
     if (isBrowse && selectedStation?.value && selectedMiddleCategory?.value && selectedSubCategory?.value) {
      fetchRegistrationIdBySlectedValues(selectedStation?.value , selectedMiddleCategory.value , selectedSubCategory.value);
    }
  }, [isBrowse, selectedStation, selectedMiddleCategory, selectedSubCategory])

  const fetchRegistrationIdBySlectedValues = async (station, middleCategory, subCategory) => {
    try {
      console.log("middleCategory", middleCategory); // ST-005
      const response = await axios.get(
...
/*   const registrationOptions = registrationIds.map(id => ({
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (new Date(formData.leasePeriodStartDate) > new Date(formData.leasePeriodEndDate)) {
        Swal.fire({
          title: "Error",
      ...
      const allFileNames = files
        .map(f => (f.isExisting ? f.serverPath.split("/").pop() : f.file?.name))
        .filter(Boolean)
        .join("@");
      formDataWithFile.append("filesNamesJoined", allFileNames);

      if(isUpdate){
        await axios.put(`http://localhost:3000/api/vehicle/update/${vehicleId}`, formDataWithFile,
          {
        ...
        await axios.post( "http://localhost:3000/api/vehicle/create", formDataWithFile,
          {
            headers: { "Content-Type": "multipart/form-data" }
          }
        );
      ...
      Swal.fire({
        title: "Success!",
        text: `Vehicle form ${isUpdate ? "updated" : "submitted"} successfully.`,
        icon: "success",
        confirmButtonText: "OK",
      });
      // Reset form
      ...
      const errorMsg = error.response?.data?.error || "Failed to submit vehicle form.";
      Swal.fire({
        title: "Error!",
        text: errorMsg,
        icon: "error",
    ...
    <div className="page">
      <div className="container">
            ...
            <div className="paired-section">
              ...
              <div className="paired-column">
                ...
                <div className="form-group">
                  <label className="form-label">Registration Id</label>
                  {
                    isUpdate || isBrowse ? (
                      ...
                      (
                        <input className="form-control" name="registrationId" value={formData.registrationId} onChange={handleChange} />
                      )
                  }
                ...
                <div className="form-group">
                  ...
                  <input className="form-control" name="taxationClass" value={formData.taxationClass} onChange={handleChange} />
                </div>
                {/* Year */}
                <div className="form-group">
                ...
                <div className="form-group">
                  ...
                  <input className="form-control" name="chasisNo" value={formData.chasisNo} onChange={handleChange} />
                </div>
                {/* Engin No */}
                <div className="form-group">
                  <label className="form-label">Engin No</label>
                  <input className="form-control" name="engineNo" value={formData.engineNo} onChange={handleChange} />
                </div>
                {/* Color */}
                <div className="form-group">
                  <label className="form-label">Color</label>
                  <input className="form-control" name="color" value={formData.color} onChange={handleChange} />
                </div>
                {/* Seating Capacity */}
                <div className="form-group">
                  <label className="form-label">Seating Capacity</label>
                  <input type="number" className="form-control" name="seatingCapacity" value={formData.seatingCapacity} onChange={handleChange} />
                </div>
                {/* Cylinder Capacity */}
                <div className="form-group">
                  <label className="form-label">Cylinder Capacity</label>
                  <input type="number" className="form-control" name="cylinderCapacity" value={formData.cylinderCapacity} onChange={handleChange} />
                </div>
                {/* Make */}
                <div className="form-group">
              ...
              <div className="paired-column">
                ...
                <div className="form-group">
                  ...
                  <input className="form-control" name="bodyType" value={formData.bodyType} onChange={handleChange} />
                </div>
                {/* Weight Unlader */}
                <div className="form-group">
                  <label className="form-label">Weight Unlader</label>
                  <input type="number" className="form-control" name="weightUnlader" value={formData.weightUnlader} onChange={handleChange} />
                </div>
                {/* Weight Gross */}
                <div className="form-group">
                  <label className="form-label">Weight Gross</label>
                  <input type="number" className="form-control" name="weightGross" value={formData.weightGross} onChange={handleChange} />
                </div>
                {/* Height */}
                <div className="form-group">
                ...
                <div className="form-group">
                  ...
                  <input type="number" className="form-control" name="horsePower" value={formData.horsePower} onChange={handleChange} />
                </div>
                {/* Fuel Type */}
                <div className="form-group">
                ...
                <div className="form-group">
                  ...
                  <input type="text" className="form-control" name="licenceRenewal" value={formData.licenceRenewal} onChange={handleChange} />
                </div>
                {/* Insurance Renewal */}
                <div className="form-group">
                  <label className="form-label">Insurance Renewal</label>
                  <input type="text" className="form-control" name="insuranceRenewal" value={formData.insuranceRenewal} onChange={handleChange} />
                </div>
                {/* Purchase Type*/}
                <div className="form-group">
                ...
                <div className="form-group">
                  ...
                  <input type="date" className="form-control" name="leasePeriodStartDate" value={formData.leasePeriodStartDate} onChange={handleChange} />
                </div>
                {/* Lease Period End Date */}
                <div className="form-group">
                  <label className="form-label">Lease Period End Date</label>
                  <input type="date" className="form-control" name="leasePeriodEndDate" value={formData.leasePeriodEndDate} onChange={handleChange} />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="paired-section">
            <div className="paired-column">
              ...
              <div className="form-group">
                ...
                <input type="number" className="form-control" name="purchasedPrice" value={formData.purchasedPrice} onChange={handleChange} />
              </div>
              {/* Total Amount */}
              <div className="form-group">
                <label className="form-label">Total Amount</label>
                <input type="number" className="form-control" name="totalAmount" value={formData.totalAmount} onChange={handleChange} />
              </div>
            </div>
            <div className="paired-column">
             {/*  Receive Type */}
              ...
              <div className="form-group">
                ...
                <input type="date" className="form-control" name="purchasedDate" value={formData.purchasedDate} onChange={handleChange} />
              </div>
              {/* Vote No */}
              <div className="form-group">
        ...
        <div className="d-flex justify-content-center gap-2 mt-3">
          ...
          <button className="button button-browse"
            type="button"
